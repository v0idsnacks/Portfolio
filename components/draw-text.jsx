...
export function DrawText({ text, className = "", duration = 0.08, delay = 0.5, stagger = 0.08 }) {
  const containerRef = useRef(null)
  const [displayChars, setDisplayChars] = useState(text.split("").map(() => ""))
  const [activeIndices, setActiveIndices] = useState(text.split("").map(() => false))
  const [flippingIndices, setFlippingIndices] = useState(text.split("").map(() => false))
